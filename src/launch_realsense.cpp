...
void RGB_image_thread(rs2::pipeline &pipe, rs2::align &align) {
  auto RGB_node = std::make_shared<RgbImage>(pipe, align);
  rclcpp::spin(RGB_node);
}

// Create a publisher thread for the depth images
void depth_image_thread(rs2::pipeline &pipe, rs2::align &align) {
  auto depth_node = std::make_shared<DepthImage>(pipe,align);
...
int main(int argc, char *argv[]) {
  ...
  std::thread RGB(RGB_image_thread, std::ref(pipe),std::ref(align_to));
  std::thread depth(depth_image_thread, std::ref(pipe),std::ref(align_to));
